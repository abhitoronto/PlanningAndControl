...
void sim_pose_callback(const gazebo_msgs::ModelStates& msg)
{
	...
	*tform = tform->inverse();
	...
	currPose.x = msg.pose[19].position.x; // Robot X psotition
	currPose.y = msg.pose[19].position.y; // Robot Y psotition	

	double tempAngle = (tf::getYaw(msg.pose[19].orientation)); // Robot Yaw
	...
	//ROS_INFO("Update Robot Positionnnn: %f %f %f\n", currPose.x, currPose.y, currPose.angle);
	    ...
	    post_path.poses.resize(path.size());

	    for(int i = 0; i < path.size(); i ++)
	    {
		post_path.poses[i] = path[i];
	    }    
	    post_path.header.frame_id = "map";

	    path_pub.publish(post_path);
...
void robotController(const pose2d_t& waypoint, geometry_msgs::Twist &move_cmd, float goal_angle)
{
	...
	if (fabs(currPose.angle - goal_angle) < 0.02) 
	{
	...
	if ((currPose.angle - goal_angle) > 0.01) 
		move_cmd.angular.z = -0.5*fabs(currPose.angle - goal_angle);
	if ((currPose.angle - goal_angle) < 0.01) 
		move_cmd.angular.z = 0.5*fabs(currPose.angle - goal_angle);
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
		...
		bool pathFound = plan2dPath(currPose,
                                            goals[currGoal + 1], 
                                            wayPoints, 
                                            gridMap,
                                            NUM_RAND_POINTS, 
	...
	ROS_INFO("WPX: %d WPY: %d\n", wayPoints.size(), wayPoints.size());

		if(wayPoints.size() > 0)
		{
			for(uint32_t i = 0; i < wayPoints.size()-1; i++)
			{
				ROS_INFO("WPX: %f WPY: %f\n", wayPoints[i].x, wayPoints[i].y); 
			}

			for(uint32_t i = 0; i < wayPoints.size()-1; i++)
			{

				pose2d_t samplePoint = wayPoints[i];
				//samplePoint.x = (wayPoints[i]).x;
				//samplePoint.y = (wayPoints[i]).y;
				...
				float goal_angle;
				if (fabs(samplePoint.x - currPose.x) < 0.01)
				{
				...
				{
					//first quadrant
					if (samplePoint.x >= currPose.x && samplePoint.y >= currPose.y)
						goal_angle = atan((samplePoint.y - currPose.y)/(samplePoint.x - currPose.x));
					else if (samplePoint.x <= currPose.x && samplePoint.y >= currPose.y)
						goal_angle = 3.1415926 + atan((samplePoint.y - currPose.y)/(samplePoint.x - currPose.x));
					else if (samplePoint.x <= currPose.x && samplePoint.y <= currPose.y)
						goal_angle = 3.1415926 + atan((samplePoint.y - currPose.y)/(samplePoint.x - currPose.x));
					else if (samplePoint.x >= currPose.x && samplePoint.y <= currPose.y)
						goal_angle = atan((samplePoint.y - currPose.y)/(samplePoint.x - currPose.x)) + 6.2332;
				...
				while (fabs(currPose.angle - goal_angle) > 0.1)
				{
					...
					ROS_INFO("Current Angle: %f Goal Angle: %f Angle Difference: %f\n", currPose.angle, goal_angle, difference);
				}
				...
				//ROS_INFO("before velocity control\n");
				float carrotTime = 0;
				while(fabs(currPose.x - samplePoint.x) > 0.1 || fabs((currPose.y - samplePoint.y) > 0.1))
				{
					ros::spinOnce();   //Check for new messages
					sleep(1);
					move_cmd.linear.x = 3*fabs(samplePoint.x-currPose.x);
					move_cmd.linear.y = 3*fabs(samplePoint.y-currPose.y);
					velocity_publisher.publish(move_cmd); // Publish the command velocity
					ROS_INFO("X Vel: %f\n Y Vel: %f WPX: %f WPY: %f\n", move_cmd.linear.x, move_cmd.linear.y, samplePoint.x, samplePoint.y);
